`timescale 1ns/10ps
`define CYCLE  100      // Modify your clock period here 

`ifdef tb01
  `define INFILE1 "DAT/IN01.DAT"
  `define INFILE2 "DAT/EXPECT01.DAT"
`endif

`ifdef tb02
  `define INFILE1 "DAT/IN02.DAT"
  `define INFILE2 "DAT/EXPECT02.DAT"
`endif

`ifdef tb03
  `define INFILE1 "DAT/IN03.DAT"
  `define INFILE2 "DAT/EXPECT03.DAT"
`endif

`ifdef tb04
  `define INFILE1 "DAT/IN04.DAT"
  `define INFILE2 "DAT/EXPECT04.DAT"
`endif

`ifdef tb05
  `define INFILE1 "DAT/IN05.DAT"
  `define INFILE2 "DAT/EXPECT05.DAT"
`endif

`ifdef tb06
  `define INFILE1 "DAT/IN06.DAT"
  `define INFILE2 "DAT/EXPECT06.DAT"
`endif

`ifdef tb07
  `define INFILE1 "DAT/IN07.DAT"
  `define INFILE2 "DAT/EXPECT07.DAT"
`endif

`ifdef tb08
  `define INFILE1 "DAT/IN08.DAT"
  `define INFILE2 "DAT/EXPECT08.DAT"
`endif

`ifdef tb09
  `define INFILE1 "DAT/IN09.DAT"
  `define INFILE2 "DAT/EXPECT09.DAT"
`endif

`ifdef tb10
  `define INFILE1 "DAT/IN10.DAT"
  `define INFILE2 "DAT/EXPECT10.DAT"
`endif

`ifdef tb11
  `define INFILE1 "DAT/IN11.DAT"
  `define INFILE2 "DAT/EXPECT11.DAT"
`endif

`ifdef tb12
  `define INFILE1 "DAT/IN12.DAT"
  `define INFILE2 "DAT/EXPECT12.DAT"
`endif

`ifdef tb13
  `define INFILE1 "DAT/IN13.DAT"
  `define INFILE2 "DAT/EXPECT13.DAT"
`endif

`ifdef tb14
  `define INFILE1 "DAT/IN14.DAT"
  `define INFILE2 "DAT/EXPECT14.DAT"
`endif

`ifdef tb15
  `define INFILE1 "DAT/IN15.DAT"
  `define INFILE2 "DAT/EXPECT15.DAT"
`endif

`ifdef tb16
  `define INFILE1 "DAT/IN16.DAT"
  `define INFILE2 "DAT/EXPECT16.DAT"
`endif

`ifdef tb17
  `define INFILE1 "DAT/IN17.DAT"
  `define INFILE2 "DAT/EXPECT17.DAT"
`endif

`define End_CYCLE  100000000

module test;
parameter INPUT_DATA = `INFILE1;
parameter EXPECT_DATA = `INFILE2;
reg     clk;
reg     reset;
reg     nt;
reg     [7:0] xi, yi;

wire    [7:0] xo, yo;
wire    po;
wire    busy;


integer i, j, k, l, m,  out_f, err,cor, pattern_num, total_num, total_cycle_num, loop_num;
integer a, b, c, d;
reg [15:0]  data_base [0:304128];
reg [15:0]  data_base_expect [0:304128];
reg [15:0]  data_tmp_expect;
reg [15:0]  data_tmp_i, data_tmp_i1, data_tmp_i2, data_tmp_i3;


trapezoid top(clk, reset, nt, xi, yi, busy, po, xo, yo);

//initial $sdf_annotate("chip.sdf",top);

initial	$readmemh( INPUT_DATA,  data_base);
initial	$readmemh( EXPECT_DATA,  data_base_expect);

initial begin
$fsdbDumpfile("trapezoid.fsdb");
$fsdbDumpvars;

    clk   = 1'b1;
    reset = 1'b0;
    nt    = 1'b0;
    xi    = 8'bz;
    yi    = 8'bz;
   l = 0;
   i = 0;
   j = 0;
   k = 0;
   err = 0;
   cor = 0;
   pattern_num = 1 ; 
   total_num = 0 ;
end

initial begin

   out_f = $fopen("OUT.DAT");
   if (out_f == 0) begin
        $display("Output file open error !");
        $finish;
   end
end

always begin #(`CYCLE/2)  clk = ~clk;
end


initial begin 
   @(negedge clk)  
   reset = 1'b1;
   $display ("\n******START to VERIFY the Trapezoid Rendering Enginen OPERATION  ******\n");
   #(`CYCLE -0.1)
          reset = 1'b0;
   
    for  (i = 0; i < 4; i = i + k) begin
    if(busy ==1'b1) begin
      @(negedge clk)
           nt =1'b0;
           k  =0;
    end
    else begin   // else begin
           k  = 4;
      @(negedge clk)
           nt =1'b1;
      
      #(`CYCLE*0.3)  data_tmp_i = data_base[i] ;
           xi = data_tmp_i[15:8];
           yi = data_tmp_i[7:0];
      @(posedge clk)
      #(`CYCLE*0.2) 
           xi = 8'bz; 
           yi = 8'bz; 
      @(negedge clk)
           nt = 1'b0;
      #(`CYCLE*0.3)  data_tmp_i1 = data_base[i+1] ;
           xi = data_tmp_i1[15:8];
           yi = data_tmp_i1[7:0];
      @(posedge clk)
      #(`CYCLE*0.2) 
           xi = 8'bz; 
           yi = 8'bz; 
      @(negedge clk)
      #(`CYCLE*0.3)  data_tmp_i2 = data_base[i+2] ;
           xi = data_tmp_i2[15:8];
           yi = data_tmp_i2[7:0];
      @(posedge clk)
      #(`CYCLE*0.2) 
           xi = 8'bz; 
           yi = 8'bz; 

      @(negedge clk)
      #(`CYCLE*0.3)  data_tmp_i3 = data_base[i+3] ;
           xi = data_tmp_i3[15:8];
           yi = data_tmp_i3[7:0];
      @(posedge clk)
      #(`CYCLE*0.2) 
           xi = 8'bz; 
           yi = 8'bz; 
      
  $display("Waiting for the rendering operation of the trapezoid points with:"); 
    $display("(xul, yu)=(%h, %h)",data_tmp_i[15:8], data_tmp_i[7:0]); 
    $display("(xur, yu)=(%h, %h)",data_tmp_i1[15:8], data_tmp_i1[7:0]); 
    $display("(xdl, yd)=(%h, %h)",data_tmp_i2[15:8], data_tmp_i2[7:0]); 
    $display("(xdr, yd)=(%h, %h)",data_tmp_i3[15:8], data_tmp_i3[7:0]); 
  end //else end 
 end //for end
end 

initial begin
	loop_num = data_base_expect[0];
	$display("loop_num = %d",loop_num);
end

always @ ( posedge clk) begin
  if (po ==1'b1) begin 
     data_tmp_expect = data_base_expect[l+1]; 
     if ((xo !== data_tmp_expect[15:8])|| (yo!== data_tmp_expect[7:0])) begin
         $display("ERROR at %d:xo=(%h) yo=(%h)!=expect xo=(%h), yo=(%h)",l 
         ,xo, yo, data_tmp_expect[15:8], data_tmp_expect[7:0]);
          err = err + 1 ;   
         end
	 else begin 
		 cor = cor+1;
		 //$display("CORRE at %d:xo=(%h) yo=(%h)", l, data_tmp_expect[15:8], data_tmp_expect[7:0]);
	 end
       $fdisplay(out_f,"%h%h",xo,yo); 
        l = l + 1;

	 end
    //if( l == 162 ) begin
    if( l == loop_num ) begin
      if (err == 0)
         $display("PASS! All data have been generated successfully!");
      else begin
      $display("---------------------------------------------");
         $display("There are %d errors!", err);
		 $display("There are %d correct!", cor);
		 $display("You need  %d correct!", loop_num);
      $display("---------------------------------------------");
      end
      $display("---------------------------------------------");
      total_num = total_cycle_num * `CYCLE ;
      $display("Total delay: %d ns", total_num );
      $display("---------------------------------------------");
      $finish;
    end
end

always @ ( posedge clk) begin
   if (reset == 1'b1) 
      total_cycle_num = 0 ;
   else 
      total_cycle_num = total_cycle_num + 1 ;
end 

initial  begin
 #`End_CYCLE ;
  $display("-----------------------------------------------------\n");
  $display("Error!!! The simulation can't be terminated under normal operation!\n");
  $display("There are %d errors!", err);
  $display("There are %d correct!", cor);
  $display("You need  %d correct!", loop_num);
  $display("-------------------------FAIL------------------------\n");
  $display("-----------------------------------------------------\n");
  $finish;
end

endmodule
